...
public class DiffTableIndexesDeserializer extends AbstractDiffJsonDeserializer<DiffTableIndexes> {
  ...
  public DiffTableIndexes deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException {
    ...
    if (!jsonNode.isArray()) {
      if (ifNull(jsonNode)) {
        ...
        throw from(jp, jsonNode, DiffTableIndexes.class);
      }
    }
    ...
    for (JsonNode node : jsonNodeList) {
      tableIndexList.add(JacksonUtils.toObj(node.toString(), typeReference));
    }
